...
pub async fn client() -> Result<(), String> {
    ...
    let (mut read, mut writer) = client.into_split();

    let client_read =
        tokio::spawn(async move { tokio::io::copy(&mut read, &mut tokio::io::stdout()).await });
...
pub async fn server() -> Result<(), String> {
    ...
    let (mut read, mut writer) = handle.into_split();
